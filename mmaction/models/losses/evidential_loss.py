...
def my_dice_loss(logits, targets):
...
def dce_eviloss(p, alpha, c, global_step, annealing_step):
    # L_dice =  TDice(alpha,p,criterion_dl)
    # L_dice = softmax_dice(alpha, p)
    pred_prob = alpha / alpha.sum(dim=1, keepdim=True)

    L_dice = my_dice_loss(pred_prob, p)
...
def KL(alpha, c):
    ...
    beta = torch.ones((1, c)).cuda(alpha.device.index)
...
class Eviloss_combination(nn.Module):
    def __init__(self, class_num=2, annealing_step=50):
        super(Eviloss_combination, self).__init__()
        self.class_num = class_num
        self.annealing_step = annealing_step

    def forward(self, pred_list, target, global_step):
        # ret = pred_list[:-1] are the four-phase evidences, pred_list[-1] is the combination evidence
        evidence_losses = []
        dice_losses = []
        kl_losses = []
        total_loss = 0
        for ins_pred in pred_list:
            ins_pred = torch.split(ins_pred, 2, dim=1)
            # ev_loss, dice_loss, kl_loss = ce_loss(target, ins_pred, self.class_num, global_step, self.annealing_step)
            for idx, t_y_hat in enumerate(ins_pred):
                t_y = target[:, idx]
                t_y_hot = one_hot_embedding(t_y, 2)
                total_loss += edl_log_loss(t_y_hat, t_y_hot.float(), global_step, 2, self.annealing_step, t_y.device)

        return torch.mean(total_loss)


if __name__ == '__main__':
    y = torch.randint(0,1,(3,224,224))
    backbone_out = torch.rand((3, 2, 224, 224))
    backbone_out2 = torch.rand((3, 2, 224, 224))

    evidence = F.softplus(backbone_out)
    alpha = evidence + 1

    evidence2 = F.softplus(backbone_out2)
    alpha2 = evidence2 + 1
    ...
    temp = alpha[0,0,:] + alpha[0,1,:]
    temp = evidence[0] / temp

    b1, u1 = get_subjective_info(alpha, evidence, 2)
    b2, u2 = get_subjective_info(alpha2, evidence2, 2)

    print(b1[0,:,0, 0], u1[0,0,0,0], b1[0,:,0, 0].sum() + u1[0,0,0,0])
    ...
    bb = torch.bmm(b1.view(-1, classes, 1), b2.view(-1, 1, classes))
    bb_sum = torch.sum(bb, dim=(1, 2), out=None)
    bb_diag = torch.diagonal(bb, dim1=-2, dim2=-1).sum(-1)
    K = bb_sum - bb_diag
    C = (1 - K).view(-1, 1).view(b1[:,0].shape)

    C2 = (1 - (torch.mul(b1[:, 0], b2[:, 1])+torch.mul(b2[:, 0], b1[:, 1])))

    temp1_b = (b1b2 + b1u2 + b2u1) / C.unsqueeze(1)
    temp2_b = (b1b2 + b1u2 + b2u1) / C2.unsqueeze(1)

    temp1_u = u1u2 / C.unsqueeze(1)
    temp2_u = u1u2 / C2.unsqueeze(1)

    print(temp1_b[0,:,0, 0], temp1_u[0,0,0,0], temp1_b[0,:,0, 0].sum() + temp1_u[0,0,0,0])
    print(temp2_b[0,:,0, 0], temp2_u[0,0,0,0], temp2_b[0,:,0, 0].sum() + temp2_u[0,0,0,0])


    # temp1 = ret[0,0,0]
    # temp2 = b1[0,0,0,0] * b2[0,1,0,0]
    pred_list = [backbone_out]*5
    ret = Eviloss_combination(pred_list, y.to(torch.int64), 10)
    print(ret)

    # loss = dce_eviloss(y.to(torch.int64), alpha, classes, global_step, lambda_epochs)
