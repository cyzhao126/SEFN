...
class multiview_SampleFrames:
    ...
    def _get_train_clips(self, num_frames, per_view1, per_view2):
        """Get clip offsets in train mode.
        ...
        clip_offsets = [0, 0]
        if avg_interval > 0:
            if len(per_view1) > 1 and len(per_view2) > 1:
                view1 = [per_view1[i:i + 2] for i in range(0, len(per_view1), 2)]
                view2 = [per_view2[i:i + 2] for i in range(0, len(per_view2), 2)]
                range1 = []
                range2 = []
                for one in view1:
                    range1.extend(np.arange(one[0], one[1]+1, 1))
                for one in view2:
                    range2.extend(np.arange(one[0], one[1]+1, 1))
                clip_offsets[0] = random.sample(range1, 1)
                clip_offsets[1] = random.sample(range2, 1)
                # base_offsets = np.arange(self.num_clips) * avg_interval
                # clip_offsets = base_offsets + np.random.randint(
                #     avg_interval, size=self.num_clips)
            elif len(per_view1) > 1 and len(per_view2) == 1:
                view1 = [per_view1[i:i + 2] for i in range(0, len(per_view1), 2)]
                range1 = []
                for one in view1:
                    range1.extend(np.arange(one[0], one[1]+1, 1))
                clip_offsets[0] = random.sample(range1, 1)
                base_offsets = np.arange(self.num_clips) * avg_interval
                clip_offsets[1] = base_offsets + np.random.randint(
                    avg_interval, size=self.num_clips)
            elif len(per_view1) == 1 and len(per_view2) > 1:
                view2 = [per_view2[i:i + 2] for i in range(0, len(per_view2), 2)]
                range2 = []
                ...
                clip_offsets[1] = random.sample(range2, 1)
                base_offsets = np.arange(self.num_clips) * avg_interval
                clip_offsets[0] = base_offsets + np.random.randint(
                    avg_interval, size=self.num_clips)
            else:
                base_offsets = np.arange(self.num_clips) * avg_interval
                clip_offsets[0] = base_offsets + np.random.randint(
            ...
            clip_offsets[0] = np.sort(
                np.random.randint(
                    num_frames - ori_clip_len + 1, size=self.num_clips))
            clip_offsets[1] = np.sort(
                np.random.randint(
                    num_frames - ori_clip_len + 1, size=self.num_clips))
        elif avg_interval == 0:
            ratio = (num_frames - ori_clip_len + 1.0) / self.num_clips
            clip_offsets[0] = np.around(np.arange(self.num_clips) * ratio)
            clip_offsets[1] = np.around(np.arange(self.num_clips) * ratio)
        else:
            clip_offsets[0] = np.zeros((self.num_clips, ), dtype=np.int)
            clip_offsets[1] = np.zeros((self.num_clips, ), dtype=np.int)

        return clip_offsets

    def _get_test_clips(self, num_frames, per_view1, per_view2):
        """Get clip offsets in test mode.
    ...
    def _sample_clips(self, num_frames, per_view1, per_view2):
        """Choose clip offsets for the video in a given mode.
        ...
        """
        if self.test_mode:
            clip_offsets = self._get_test_clips(num_frames, per_view1, per_view2)
        else:
            clip_offsets = self._get_train_clips(num_frames, per_view1, per_view2)
    ...
    def __call__(self, results):
        ...
        total_frames = results['total_frames']
        total_frames = int(total_frames)
        per_view1 = results['per_view1']
        if len(per_view1) > 1:
            per_view1 = list(map(int, per_view1))
        per_view2 = results['per_view2']
        if len(per_view2) > 1:
            per_view2 = list(map(int, per_view2))
        if self.frame_uniform:  # sthv2 sampling strategy
            assert results['start_index'] == 0
            frame_inds = self.get_seq_frames(total_frames)
        else:
            clip_offsets = self._sample_clips(total_frames, per_view1, per_view2)
            frame_ind0 = clip_offsets[0] + np.arange(
                self.clip_len)[None, :] * self.frame_interval
            frame_ind1 = clip_offsets[1] + np.arange(
                self.clip_len)[None, :] * self.frame_interval
            frame_ind0 = np.concatenate(frame_ind0)
            frame_ind1 = np.concatenate(frame_ind1)

            if self.temporal_jitter:
                perframe_offset0 = np.random.randint(
                    self.frame_interval, size=len(frame_ind0))
                perframe_offset1 = np.random.randint(
                    self.frame_interval, size=len(frame_ind1))
                frame_ind0 += perframe_offset0
                frame_ind1 += perframe_offset1

            frame_ind0 = frame_ind0.reshape((-1, self.clip_len))
            frame_ind1 = frame_ind1.reshape((-1, self.clip_len))
            if self.out_of_bound_opt == 'loop':
                frame_ind0 = np.mod(frame_ind0, total_frames)
                frame_ind1 = np.mod(frame_ind1, total_frames)
            elif self.out_of_bound_opt == 'repeat_last':
                safe_ind0 = frame_ind0 < total_frames
                unsafe_ind0 = 1 - safe_ind0
                last_ind0 = np.max(safe_ind0 * frame_ind0, axis=1)
                new_ind0 = (safe_ind0 * frame_ind0 + (unsafe_ind0.T * last_ind0).T)
                frame_ind0 = new_ind0
                safe_ind1 = frame_ind1 < total_frames
                unsafe_ind1 = 1 - safe_ind1
                last_ind1 = np.max(safe_ind1 * frame_ind1, axis=1)
                new_ind1 = (safe_ind1 * frame_ind1 + (unsafe_ind1.T * last_ind1).T)
                frame_ind1 = new_ind1
            else:
                raise ValueError('Illegal out_of_bound option.')

            start_index = results['start_index']
            frame_ind0 = np.concatenate(frame_ind0) + start_index
            frame_ind1 = np.concatenate(frame_ind1) + start_index

        results['frame_ind0'] = frame_ind0.astype(np.int)
        results['frame_ind1'] = frame_ind1.astype(np.int)
        results['clip_len'] = self.clip_len
        results['frame_interval'] = self.frame_interval
        results['num_clips'] = self.num_clips
        return results
    ...
    def _get_train_clips(self, num_frames, per_view1, per_view2):
        ...
        if avg_interval > 0:
            if len(per_view1) > 1 and len(per_view2) > 1:
                ...
                for one in view2:
                    range2.extend(np.arange(one[0], one[1]+1, 1))
                ranges.extend(range1)
                ranges.extend(range2)
                clip_offsets[0] = random.sample(ranges, 1)
                ranges.remove(clip_offsets[0])
                clip_offsets[1] = random.sample(ranges, 1)
                # base_offsets = np.arange(self.num_clips) * avg_interval
                # clip_offsets = base_offsets + np.random.randint(
                #     avg_interval, size=self.num_clips)
            elif len(per_view1) > 1 and len(per_view2) == 1:
...
@PIPELINES.register_module()
class doubleinone_SampleFrames:
    """Sample frames from the video.
...
@PIPELINES.register_module()
class randomdouble_SampleFrames:
    """Sample frames from the video.
...
class multiviewOpenCVDecode:
    ...
    def __call__(self, results):
        ...
        if results['frame_ind0'].ndim != 1:
            results['frame_ind0'] = np.squeeze(results['frame_ind0'])
        if results['frame_ind1'].ndim != 1:
            results['frame_ind1'] = np.squeeze(results['frame_ind1'])

    # for frame_ind in results['frame_inds']:
    #     frame_ind0 = results['frame_ind0']
    #     frame_ind1 = results['frame_ind1']

        # last frame may be None in OpenCV
        for frame_ind in results['frame_ind0']:
            cur_frame = container[frame_ind]
            # last frame may be None in OpenCV
            while isinstance(cur_frame, type(None)):
                frame_ind -= 1
                cur_frame = container[frame_ind]
            imgs.append(cur_frame)
        for frame_ind in results['frame_ind1']:
            cur_frame = container[frame_ind]
            # last frame may be None in OpenCV
            while isinstance(cur_frame, type(None)):
                frame_ind -= 1
...
class RawFrameDecode:
    ...
    def __call__(self, results):
        ...
        filename_tmpl = results['filename']
