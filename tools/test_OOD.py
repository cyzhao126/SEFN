...
import sys
# print(sys.path)
# sys.path.append('/data4/hucf/code/sewer/sewer_qv')
import csv
...
from tools.edl_loss import ood_evaluation
...
def parse_args():
    ...
    parser.add_argument('--config',
                        default='C:/zcy/Video-Swin-Transformer-master/Video-Swin-Transformer-master/configs/recognition/swin/pareto_OOD_least4.py',
                        help='test config file path')
    parser.add_argument('--checkpoint',
                        default='C:/zcy/video_swin_transformer_ssh/data/TRACK1/single_pareto_OOD_least4_lr_0002_batch_12/epoch_75.pth',
...
def save_outputs(val_step_outputs, checkpoint):
    pred_list = val_step_outputs[::4]
    pred_list = [i[0] for i in pred_list]
    gt_list = val_step_outputs[1::4]
    uncertainty_list = val_step_outputs[2::4]
    uncertainty_list = [i[0] for i in uncertainty_list]
    ood_label = val_step_outputs[3::4]
    pred_list = np.array(pred_list)
    gt_list = np.array(gt_list)
    uncertainty_list = np.array(uncertainty_list)
    ood_label = np.array(ood_label)
    ...
    preds_view1 = np.expand_dims(preds_view1, axis=1)
    gts_view1 = gt_list[:, 0]
    uncertainties_view1 = uncertainty_list[:, :]
    uncertainties_view1 = np.expand_dims(uncertainties_view1, axis=1)
    ood_label1 = ood_label[:, 0, :]
    row1 = np.concatenate((preds_view1, gts_view1, uncertainties_view1), axis=1)
    list1 = row1.tolist()
    ood_label1 = ood_label1.tolist()
    for i in range(0, len(list1)):
        list1[i].append(ood_label1[i])

    firstrow = ['alpha', 'gt', 'uncertanty', 'ood_gt']
...
def validation_epoch_end(val_step_outputs, checkpoint):
    ...
    uncertainties_view1 = np.expand_dims(uncertainties_view1, axis=1)
    ood_label1 = ood_label[:, 0, :]
    new_view1, main_view1, auxillary_view1 = evaluation(preds_view1, gts_view1)
    ood_metrics1 = ood_evaluation(uncertainties_view1, ood_label1, preds_view1, gts_view1)
    metrics_ret_view1 = [new_view1, main_view1, auxillary_view1, ood_metrics1]

    log_dir = 'C:/zcy/Video-Swin-Transformer-master/Video-Swin-Transformer-master/output/'
    metrics_view1_file = os.path.join(log_dir, 'metrics_view1_log.txt')
...
def multiview_test(model, data_loader):
...
def inference_pytorch(args, cfg, distributed, data_loader, dataset, eval_config):
    ...
    if not distributed:
        ...
        outputs = multiview_test(model, data_loader)
    else:
...
def main():
    ...
    # f = open('C:/zcy/Video-Swin-Transformer-master/Video-Swin-Transformer-master/output/test_log1.txt', 'a+')
    # sys.stdout = f
